...
///
/// <b>Milestone 018's first correction added, and its third removed, a `WakeEventId` field</b> that
/// named the wake event this request scheduled, meant to let a reader tell "not yet decided" from
/// "decided and declined" by checking whether the asked character's own `DecisionRecord` existed for
/// it. Codex's review rejected that — the asker never receives any message establishing that the
/// asked character has decided anything at all, so reading it was a private-state leak regardless of
/// how little of the record was consulted — and once the corrected derivation stopped reading it, the
/// field itself had no remaining production consumer: written, replay-compared, never read. Removed
/// rather than kept as unused linkage state; see `Session.PlayerView.Build`'s own request-resolution
/// comment for what actually decides whether a request is still outstanding.
/// </summary>
